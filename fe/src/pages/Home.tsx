...
function Home() {
  ...
  return (
    <Grid columns={4} sx={{ gap: "2rem" }}>
      <For each={installed()}>
        {(app) => (
          <Flex sx={{ backgroundColor: "rgb(0, 30, 60)", border: "1px solid #1e4976", borderRadius: "10px", flexDirection: "column", overflow: "hidden" }}>
            <img alt={app.name} src={app.image} style={{ height: "auto", width: "100%" }} />
            <Flex sx={{ padding: "1rem" }}>
              <header style={{ color: "white", "font-weight": 600, overflow: "hidden", "text-overflow": "ellipsis", "white-space": "nowrap" }}>{app.name}</header>
            </Flex>
          </Flex>
        )}
      </For>
